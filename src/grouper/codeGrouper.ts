import { GitChange, FileType, ChangeType } from '../analyzer/gitAnalyzer';
...
export class CodeGrouper {
     ...
     * @returns 分组后的变更
     */
    groupChanges(changes: GitChange[]): ChangeGroup[] {
        // 首先按文件类型分组
        const groupsByFileType = this.groupByFileType(changes);
        
        // 然后对每个文件类型组进行更细粒度的分组
        const finalGroups: ChangeGroup[] = [];
     ...
     * @returns 按文件类型分组的变更
     */
    private groupByFileType(changes: GitChange[]): Record<string, GitChange[]> {
        const groups: Record<string, GitChange[]> = {};
     ...
     * @returns 按功能模块分组的变更
     */
    private groupByModule(changes: GitChange[]): ChangeGroup[] {
        const groups: ChangeGroup[] = [];
        
        // 模块路径映射
        const moduleMap: Record<string, GitChange[]> = {};
     ...
     * @returns 分组类型
     */
    private determineGroupType(changes: GitChange[]): GroupType {
        // 统计变更类型
        const changeTypeCount: Record<string, number> = {
            [ChangeType.ADDED]: 0,
        ...
        // 根据变更类型确定分组类型
        if (changeTypeCount[ChangeType.ADDED] > 0 && changeTypeCount[ChangeType.MODIFIED] === 0 && changeTypeCount[ChangeType.DELETED] === 0) {
            return GroupType.NEW_FEATURE;
        } else if (changeTypeCount[ChangeType.DELETED] > 0 && changeTypeCount[ChangeType.ADDED] === 0) {
     ...
     * @returns 是否包含bug修复
     */
    private containsBugFix(changes: GitChange[]): boolean {
     ...
     * @returns 是否包含重构
     */
    private containsRefactoring(changes: GitChange[]): boolean {
        // 简单检查变更是否主要是代码结构调整
        // 重构通常表现为修改行数较多但功能不变
        let totalAdded = 0;
        let totalDeleted = 0;
     ...
     * @returns 分组ID
     */
    private generateGroupId(modulePath: string, groupType: GroupType): string {
     ...
     * @returns 分组描述
     */
    private generateGroupDescription(changes: GitChange[]): string {
