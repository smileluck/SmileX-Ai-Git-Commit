...
class CommitMessageGenerator {
    ...
    constructor() {
        ...
        this.aiEnabled = config.get('aiEnabled', true);
        this.commitTemplate = config.get('commitTemplate', '${type}(${scope}): ${subject}');
    }
    /**
     * 为变更分组生成提交消息
     * @param groups 变更分组数组
     * @returns 生成的提交消息数组
     */
    generateMessages(groups) {
        return __awaiter(this, void 0, void 0, function* () {
            const messages = [];
            for (const group of groups) {
                // 生成提交消息
                const message = yield this.generateMessageForGroup(group);
    ...
    generateMessageForGroup(group) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.aiEnabled) {
                // 使用AI生成主题
                subject = yield this.generateSubjectWithAI(group);
            }
            else {
                // 使用传统方法生成主题
     ...
     * @returns 提交类型
     */
    mapGroupTypeToCommitType(groupType) {
        switch (groupType) {
            case codeGrouper_1.GroupType.NEW_FEATURE:
                return 'feat';
            case codeGrouper_1.GroupType.BUG_FIX:
                return 'fix';
            case codeGrouper_1.GroupType.REFACTOR:
                return 'refactor';
            case codeGrouper_1.GroupType.ENHANCEMENT:
                return 'chore';
            case codeGrouper_1.GroupType.CLEANUP:
                return 'chore';
            case codeGrouper_1.GroupType.CONFIG:
                return 'config';
            default:
                return 'chore';
        }
